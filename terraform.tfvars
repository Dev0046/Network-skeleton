...
public_subnet_tags       = { Environment = "dev" }
private_subnet_cidr      = ["10.0.0.32/28", "10.0.0.48/28", "10.0.0.64/28"]
private_sub_az           = ["us-east-1a", "us-east-1a", "us-east-1a"]
private_subnet_tags      = [
  { name = "frontend", environment = "dev" },
  { name = "application", environment = "dev" },
  { name = "database", environment = "dev" }
]
igw_name                 = "my-igw"
elasticip_name           = "dev-elasticip"
public_rt_tag            = [{ name = "public-route-table", environment = "dev" }]
private_rt_tag           = [{ name = "private-route-table", environment = "dev" }]

public_nacl_ingress_rules = [
  { protocol = "-1", rule_no = 90, action = "allow", cidr_block = "0.0.0.0/0", from_port = 0, to_port = 0 },
...
alb_sg_tag               = { name = "alb-security-group", environment = "dev" }

alb_ingress_rule         = [
  { from_port = 80, to_port = 80, protocol = "tcp", cidr_blocks = ["0.0.0.0/0"], ipv6_cidr_blocks = ["::/0"] },
  { from_port = 443, to_port = 443, protocol = "tcp", cidr_blocks = ["0.0.0.0/0"], ipv6_cidr_blocks = ["::/0"] }
]
